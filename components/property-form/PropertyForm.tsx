...
// Importamos la acción de IA que creamos
import { generatePropertyDescription } from "@/app/dashboard/properties/ai-actions"; 
import ImageUploader from "./ImageUploader";
import { toast } from "sonner"; // Importante para notificaciones
...
import { Loader2, Save, X, Plus, Sparkles } from "lucide-react"; // Agregamos Sparkles
...
export default function PropertyForm({ initialData, propertyId, isEdit = false }: PropertyFormProps) {
  ...
  const [isGeneratingAI, setIsGeneratingAI] = useState(false); // Estado para la IA
  ...
  const handleAIGenerate = async () => {
    // Validamos que haya título al menos
    if (!formData.title) {
      toast.error("Please enter a Headline first to generate context.");
      return;
    }
    ...
    if (result.success && result.data) {
      setFormData((prev: any) => ({ ...prev, description: result.data }));
      toast.success("Description generated by AI!");
    } else {
      toast.error(result.error || "Error generating description");
    }
              ...
              <div className="flex flex-col gap-2">
                <div className="flex justify-end">
                  <Button 
                    ...
                    className="text-[#0048BC] border-blue-200 hover:bg-blue-50 h-7 text-xs shadow-sm"
                  >
                    {isGeneratingAI ? (
                      <>
